...
const FinanceTransformersSection = () => {
  ...
  const episodes = [{
    ...
    title: "Lisa Emme",
    description: "Gemeinsam mit Lisa Emme sprechen wir über erfolgreiche Automatisierungsprojekte und die Bedeutung von Change Management im Finance-Bereich.",
    image: "/img/lisa.jpg",
    ...
    description: "Katharina Herzog teilt Einblicke in die digitale Transformation eines globalen Konzerns und wie Finanzteams dabei neu zusammenarbeiten.",
        ...
        <div className="relative bg-gradient-to-br from-gray-50 to-gray-100 rounded-2xl p-8 shadow-xl" onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd}>
          <div className="grid lg:grid-cols-2 gap-8 items-center">
            ...
            <div className="relative">
              <img src={episodes[currentEpisode].image} alt={episodes[currentEpisode].alt} className={`aspect-square object-cover w-full h-full rounded-xl shadow-lg ${episodes[currentEpisode].comingSoon ? 'grayscale' : ''}`} />
              {episodes[currentEpisode].comingSoon && <div className="absolute inset-0 flex items-center justify-center rounded-xl bg-black/50 text-white text-2xl font-bold">
                  Coming Soon!
                </div>}
            </div>
          </div>
          ...
          <div className="flex justify-center items-center space-x-4 mt-8">
            ...
            <div className="flex space-x-2">
              {episodes.map((episode, index) => <button key={index} onClick={() => setCurrentEpisode(index)} className={`relative w-10 h-10 rounded-full overflow-hidden border-2 transition-all duration-300 ${index === currentEpisode ? 'border-[#003FA5] scale-105' : 'border-transparent hover:border-gray-300'}`}>
                  <img src={episode.image} alt={episode.alt} className={`w-full h-full object-cover ${episode.comingSoon ? 'grayscale' : ''}`} />
                  {episode.comingSoon && <span className="absolute inset-0 flex items-center justify-center bg-black/50 text-white text-[10px] font-bold">
                      Coming Soon!
                    </span>}
                </button>)}
            </div>
