-- SAFE AI RESEARCH SYSTEM SETUP FOR PRODUCTION
...
-- Insert default research templates (safe)
...
Please provide specific data points, statistics, and actionable insights where possible. Focus on recent developments and current market dynamics.', 
    ...
    ('Quick Research Template', 'Fast overview for general research topics', 'custom',
...
WHERE NOT EXISTS (
    SELECT 1 FROM public.research_templates 
    WHERE name = templates.name
);
...
BEGIN
  ...
  RAISE NOTICE '';
  RAISE NOTICE '🎉 You can now refresh your browser and use the AI Research system!';
END $$;